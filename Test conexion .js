...
async function diagnostico() {
  ...
  console.log(
    "🔑 Key:",
    process.env.SUPABASE_ANON_KEY ? "✅ Presente" : "❌ Falta",
  );
  console.log(
    "🔐 JWT_SECRET:",
    process.env.JWT_SECRET ? "✅ Presente" : "❌ Falta",
  );
  console.log("");
  ...
  for (const tabla of tablas) {
    try {
      ...
      if (error) {
        if (error.code === "42P01") {
          console.log(
            `❌ ${tabla.padEnd(30)} → TABLA NO EXISTE (ejecuta setup_database.sql)`,
          );
        } else if (error.code === "PGRST301") {
          console.log(
            `🔒 ${tabla.padEnd(30)} → BLOQUEADA POR RLS (ver instrucciones abajo)`,
          );
        } else {
